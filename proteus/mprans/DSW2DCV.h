...
inline double nu1(const double &g, const double &hStar, const double &hL,
                  const double &uL, const double &etaL,
                  const double &meshSizeL) {
  double speed =
...
inline double nu3(const double &g, const double &hStar, const double &hR,
                  const double &uR, const double &etaR,
                  const double &meshSizeR) {
  double speed =
...
public:
  ...
  virtual void calculateResidual( // last EDGE BASED version
      ...
      int offset_hu, int offset_hv, int offset_heta, int offset_hw,
      int stride_h, int stride_hu, int stride_hv, int stride_heta,
      int stride_hw, double *globalResidual,
...
public:
  ...
  DSW2DCV()
      : nDOF_test_X_trial_element(nDOF_test_element * nDOF_trial_element),
        ck() {
    std::cout << "Constructing DSW2DCV<CompKernelTemplate<" << nSpace << ","
  ...
  inline double maxWaveSpeedSharpInitialGuess(double g, double nx, double ny,
                                              ...
                                              double hetaL, double meshSizeL,
                                              double hR, double huR, double hvR,
                                              double hetaR, double meshSizeR,
  ...
  void FCTStep(double dt,
    ...
    register double Rneg[numDOFs], Rpos[numDOFs], hLow[numDOFs], huLow[numDOFs],
        hvLow[numDOFs], hetaLow[numDOFs], hwLow[numDOFs];
  ...
  void convexLimiting(
    ...
    register double Rneg[numDOFs], Rpos[numDOFs], Kmax[numDOFs], hLow[numDOFs],
        huLow[numDOFs], hvLow[numDOFs], hetaLow[numDOFs], hwLow[numDOFs];
    ...
    for (int i = 0; i < numDOFsPerEqn; i++) {
      ...
      for (int offset = csrRowIndeces_DofLoops[i];
        ...
        if (i != j) {
          ...
          dLow[ij] = fmax(maxWaveSpeedSharpInitialGuess(
                              g, nxij, nyij, hi, hui, hvi, hetai, meshi, hj,
                              huj, hvj, hetaj, meshj, hEps, hEps, debug) *
                              cij_norm, // hEps
                          maxWaveSpeedSharpInitialGuess(
                              g, nxji, nyji, hj, huj, hvj, hetaj, meshj, hi,
                              hui, hvi, hetai, meshi, hEps, hEps, debug) *
                              cji_norm); // hEps
          dLowii -= dLow[ij];
      ...
      for (int i = 0; i < numDOFsPerEqn; i++) {
        ...
        for (int offset = csrRowIndeces_DofLoops[i];
          ...
          if (i != j) {
            dLij =
                dLow[ij] *
                fmax(psi[i], psi[j]); // enhance the order to 2nd order. No EV
  ...
  void calculateResidual( // last EDGE BASED version
    ...
    if (SECOND_CALL_CALCULATE_RESIDUAL == 0) // This is to save some time
    {
      ...
      int ij = 0;
      register double hyp_flux_h[numDOFsPerEqn], hyp_flux_hu[numDOFsPerEqn],
      ...
      for (int i = 0; i < numDOFsPerEqn; i++) {
        ...
        for (int offset = csrRowIndeces_DofLoops[i];
          ...
          ////////////////////////
          double pTildej =
              -MGN_constantj / 3.0 * 6.0 * hj * (hetaj - std::pow(hj, 2.0));
          if (etaj >= hj) {
            pTildej = -MGN_constantj / 3.0 * 2.0 *
      ...
      for (int i = 0; i < numDOFsPerEqn; i++) {
                      ...
                      // ith_dissipative_terms
          {
            ...
            if (lstage == 0)
              ...
              dLowij = fmax(maxWaveSpeedSharpInitialGuess(
                                g, nxij, nyij, hi, hui, hvi, hetai, meshi, hj,
                                huj, hvj, hetaj, meshj, hEps, hEps, false) *
                                cij_norm,
                            maxWaveSpeedSharpInitialGuess(
                                g, nxji, nyji, hj, huj, hvj, hetaj, meshj, hi,
                                hui, hvi, hetai, meshi, hEps, hEps, false) *
                                cji_norm);
            }
            dLij =
                dLowij *
            ...
            muLowij = fmax(fmax(0., -(ui * Cx[ij] + vi * Cy[ij])),
                           fmax(0, (uj * Cx[ij] + vj * Cy[ij])));
            muLij = muLowij *
                    fmax(psi[i], psi[j]); // enhance the order to 2nd order.
            ...
            if (dLij != 0) {
              ...
              // heta component
              hetaBar_ij = -1. / (2 * dLij) *
                               ((etaj * hetaj - etai * hetai) * Cx[ij] +
                                (etaj * hetaj - etai * hetai) * Cy[ij]) +
                           0.5 * (hetaj + hetai);
              huTilde_ij = (dLij - muLij) / (2 * dLij) *
        ...
        if (LUMPED_MASS_MATRIX == 1) {
          ...
          globalResidual[offset_hv + stride_hv * i] =
              ...
              0.0; // keep 0 for now for fake 1D
...
inline DSW2DCV_base *newDSW2DCV(int nSpaceIn, int nQuadraturePoints_elementIn,
                                int nDOF_mesh_trial_elementIn,
                                int nDOF_trial_elementIn,
                                int nDOF_test_elementIn,
                                int nQuadraturePoints_elementBoundaryIn,
                                int CompKernelFlag) {
  return proteus::chooseAndAllocateDiscretization2D<DSW2DCV_base, DSW2DCV,
                                                    CompKernel>(
